...
const outcomes = [
  {
    ...
    proof: "Typical engagement returns the cost of the project in pipeline within the first quarter.",
  },
  {
    family: "Cost",
    accent: "verdigris",
    ...
    proof: "Operating teams routinely reclaim 100+ hours per week of human attention.",
  },
  {
    family: "Speed",
    accent: "inkwell",
...
const accentClass: Record<string, { dot: string; rule: string; text: string }> = {
  terracotta: {
    dot: "bg-terracotta",
    rule: "bg-terracotta",
  ...
  return (
    <section className="px-6 md:px-10 py-24 md:py-32 bg-linen/60 border-y border-inkwell/10">
      <div className="mx-auto max-w-[1320px]">
        ...
        <Reveal delay={0.05}>
          <h2 className="display-tight text-[clamp(40px,6.5vw,88px)] text-inkwell max-w-[18ch] mb-16 md:mb-20">
              ...
              <Reveal key={o.family} delay={0.08 + i * 0.07}>
                <article className="flex flex-col gap-6 h-full">
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`font-mono text-[11px] tracking-[0.2em] uppercase ${a.text}`}>
                      {o.family}
                    </span>
                  </div>
                  <h3 className="display text-[26px] md:text-[30px] leading-[1.1] text-inkwell">
                    {o.headline}
                  </h3>
                  <ul className="flex flex-col gap-3">
                    {o.bullets.map((b) => (
                      <li key={b} className="flex gap-3 text-[15.5px] leading-[1.55] text-graphite">
                        <span className={`mt-[10px] h-px w-3 ${a.rule} shrink-0`} />
                        <span>{b}</span>
                      </li>
                    ))}
                  </ul>
                  <p className="mt-auto pt-4 border-t border-inkwell/10 text-[13.5px] leading-[1.55] text-graphite italic">
                    {o.proof}
                  </p>
                </article>
              </Reveal>
